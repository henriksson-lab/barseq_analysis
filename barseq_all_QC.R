...
for(current_pool in list_pools){
  ...
  colnames(samplemeta) <- c("sampleid","barseqid","k_reads","q30")
...
for(current_pool in list_pools){
  ...
  long_cnt <- melt(as.matrix(normalized_cnt))
...
for(current_pool in list_pools){
  ...
  all_gr$adjusted_log_gr <- all_gr$log_gr - as.double(fitted_lm$fitted.values)
  ...
  all_gr$local_sd <- NA
  for(one_day in unique(all_gr$day_int)){
  ...
  #duplicates in here all_gr_percond
  ...
  for(curcondi in 1:nrow(list_cond_compare)){
    ...
    samp1 <- str_ends(all_gr_percond$cond, cond1)
